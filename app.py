...
# 🔹 NEW: Data Source Option (ONLY ADDITION)
data_option = st.sidebar.radio(
    "Choose Data Source:",
    ["Upload Bank Statement", "Use Demo Data (bank_data.xlsx)"]
)
...
# OPTION 1: Upload
if data_option == "Upload Bank Statement":
...
# OPTION 2: Demo Data
if data_option == "Use Demo Data (bank_data.xlsx)":
    demo_path = "data/bank_data.xlsx"

    if os.path.exists(demo_path):
        with st.spinner("Loading Demo Data..."):
            df = load_excel_data(demo_path)
            df = clean_data(df)
            df = add_features(df)
            df = add_category(df)
            save_to_sqlite(df)
        ...
        st.sidebar.error("Demo file not found in data/ folder")
